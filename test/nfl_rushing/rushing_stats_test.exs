defmodule NflRushing.RushingStatsTest do
  ...
  describe "get rushing stats with filter by player name" do
    test "filters the players by name" do
      assert %RushingStats{
               all_stats: [
                 %{player: "Lance Dunbar"}
               ]
             } = RushingStats.get_rushing_stats(1, nil, "Lance Dunbar")
    end

    test "filters the players by part of the name" do
      assert %RushingStats{
               all_stats: [
                 %{player: "Lance Dunbar"}
               ]
             } = RushingStats.get_rushing_stats(1, nil, "Lance")
    end

    test "case insenstive filtring" do
      assert %RushingStats{
               all_stats: [
                 %{player: "Lance Dunbar"}
               ]
    ...
    test "return multiple players if the name repeats" do
      assert %RushingStats{
               all_stats: [
                 %{player: "Jordan Howard"},
                 %{player: "Lance Dunbar"}
               ]
             } = RushingStats.get_rushing_stats(1, nil, "d")
    end
  end
end
